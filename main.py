###====== IT0011 FINAL PROJECT =======###         [!] ATTENTION [!]  ==================================  [!]
####=== TC24 Feel Good BGYO Store ===####             
#####===============================#####             Make sure to update the image path upon opening the
###==  Agero, Mikhail Gianmarko G.  ==###           program on other devices. If you want to test uploader,
####= Flaviano, Angel Gaebrielle N. =####             just use provided images in our 'itm_img' folder.
####======   Gison, Yuan Ira   ======####               
####==   Sanchez, Aaron Joshua D.  ==####         [!]  ==================================  [!] ATTENTION [!]

#                                                     [Test Account: Username: 'ACE' || Password: 'bgyo']
import tkinter as tk 
import dbs as db 
import pygame 
import os 
from PIL import Image, ImageTk 
from tkinter import ttk, messagebox, simpledialog, filedialog, StringVar, IntVar 
...
##========== [SYSTEM INITIALIZATION & MENU PAGE] ==========##
    def __init__(self, root):
        ...
        self.root.protocol("WM_DELETE_WINDOW", lambda: self.on_close(self.root))
    ...
    def on_close(self, win):
        ...
        if messagebox.askyesno("Exit", "Are you sure you want to exit?"):
            win.destroy() 
            if win != self.root: 
                self.root.destroy() ### Restarts Program
        else:
            return 
    ...
    def logout(self,win):
        if messagebox.askyesno("Logout", f"Ready to leave {self.usr_inp.get()}?"):
                win.withdraw()
        else:
            return
        self.root.deiconify()
    ...
    def back_store(self, win):
        #===== Back to Store Button Function =====# - //This Function closes the current window and restores the customer page.//
        win.withdraw()
        self.cstmr_wn.deiconify()
...
##========== [LOGIN PAGE] ==========##
    def Lgn_pg(self):
        ...
        self.lgn_wn.protocol("WM_DELETE_WINDOW", lambda: self.on_close(self.lgn_wn))
...
##========== [REGISTRATION PAGE] ==========##
    def Reg_pg(self):
        ...
        self.reg_pg.protocol("WM_DELETE_WINDOW", lambda: self.on_close(self.reg_pg))
...
##========== [ITEM MANAGEMENT/VENDOR'S PAGE] ==========##
    def Vndr_pg(self):
        ...
        self.vndr_wn.geometry("425x700")
        self.center_window(self.vndr_wn)
        self.vndr_wn.protocol("WM_DELETE_WINDOW", lambda: self.on_close(self.vndr_wn))
        ...
        ttk.Button(self.vndr_wn, text="Back to Menu", command=self.sound(lambda: self.back_menu(self.vndr_wn))).grid(row=0, column=4, sticky="we", padx=10, pady=5)

        #= Item ID Input  =#
        ttk.Label(self.vndr_wn, text="Item ID: ").grid(row=1, column=0, sticky="we",padx=10, pady=10)
        self.itm_id = ttk.Entry(self.vndr_wn)
        self.itm_id.grid(row=1, column=1, columnspan=3, sticky="we", padx=10, pady=10)

        #= Item Name Input =#
        ttk.Label(self.vndr_wn, text="Item Name: ").grid(row=2, column=0, sticky="we",padx=10, pady=10)
        self.itm_name = ttk.Entry(self.vndr_wn)
        self.itm_name.grid(row=2, column=1, columnspan=3, sticky="we", padx=10, pady=10)

        #= Item Stock Input =#
        ttk.Label(self.vndr_wn, text="Item Stock: ").grid(row=3, column=0, sticky="we",padx=10, pady=10)
        self.itm_stock = ttk.Entry(self.vndr_wn)
        self.itm_stock.grid(row=3, column=1, columnspan=3, sticky="we", padx=10, pady=10)

        #= Item Price Input =#
        ttk.Label(self.vndr_wn, text="Item Price: ").grid(row=4, column=0, sticky="we",padx=10, pady=10)
        self.itm_price = ttk.Entry(self.vndr_wn)
        self.itm_price.grid(row=4, column=1, columnspan=3, sticky="we", padx=10, pady=10)

        #= Item Image Input =#
        ttk.Label(self.vndr_wn, text="Image Preview: ").grid(row=5, column=0, sticky="we",padx=10, pady=10)
        self.itm_img = ttk.Button(self.vndr_wn, text="Upload Image", command=self.sound(self.upload_image))
        ...
        self.category_lb.grid(row=6, column=0, sticky="we",padx=10, pady=10)
        ...
        nsrt_btn.grid(row=7, column=1, columnspan=3, sticky="we", padx=10, pady=10)

        #= Update Item Button =#
        upd_btn = ttk.Button(self.vndr_wn, text="Update Item", command=self.sound(self.update_item))
        upd_btn.grid(row=8, column=1, columnspan=3, sticky="we", padx=10, pady=10)

        #= Delete Item Button =#
        del_btn = ttk.Button(self.vndr_wn, text="Delete Item", command=self.sound(self.delete_item))
        del_btn.grid(row=9, column=1, columnspan=3, sticky="we", padx=10, pady=10)

        #= Show Items Button =#
        shw_btn = ttk.Button(self.vndr_wn, text="Show Items", command=self.sound(self.show_items))
        shw_btn.grid(row=10, column=1, columnspan=3, sticky="we", padx=10, pady=10)

        #= Item Listbox =#
        self.itm_lst = tk.Listbox(self.vndr_wn)
        self.itm_lst.grid(row=11, column=0, columnspan=5, sticky="we", padx=10, pady=10)
...
##========== [SHOP/CUSTOMER'S PAGE] ==========##
    def Cstmr_pg(self):
        ...
        self.cstmr_wn.geometry("470x530")
        self.center_window(self.cstmr_wn)
        self.cstmr_wn.protocol("WM_DELETE_WINDOW", lambda: self.on_close(self.cstmr_wn))

        self.logo_lb = tk.Label(self.cstmr_wn, image=self.logo)
        self.logo_lb.grid(row=0, column=0, pady=10)
        
        #= Page Label and Back button =#
        ttk.Label(self.cstmr_wn, text=f"Hey, {self.usr_inp.get()}! Ready to shine like a star?", font=("Oswald")).grid(row=0, column=1, columnspan=2, sticky="w", padx=25, pady=5)
        ...
        ttk.Button(self.cstmr_wn, text="Logout", command=self.sound(lambda: self.logout(self.cstmr_wn))).grid(row=0, column=3, sticky="we", padx=10, pady=5)
...
##========== [CHECKOUT WINDOW] ==========##
    def load_items(self, event):
        ...
        self.cstmr_wn.withdraw()
        self.item_win = tk.Toplevel(self.cstmr_wn)
        self.item_win.title(f"Item Details - {itm[1]}")
        self.item_win.geometry("400x440")
        self.center_window(self.item_win)
        self.item_win.protocol("WM_DELETE_WINDOW", lambda: self.on_close(self.item_win))

        image_frame = tk.LabelFrame(self.item_win, text="Item Photo", padx=10, pady=10)
        ...
        info_frame = tk.Frame(self.item_win)
        ...
        btn_frame = tk.Frame(self.item_win)
        btn_frame.pack(pady=20)

        ttk.Button(btn_frame, text="Buy Now", command= self.sound(lambda: self.buy_item(itm))).pack(side="left", padx=10)

        ttk.Button(btn_frame, text="Back to Shop", command=self.sound(lambda: self.back_store(self.item_win))).pack(padx=10)
    ...
    def buy_item(self, selected_item):
        ...
        while True:
            qty = simpledialog.askinteger("Quantity", f"Enter quantity to purchase (Available: {stock}):", parent=self.item_win)
            if qty is None:
        ...
        total = qty * price
        while True:
            payment = simpledialog.askfloat("Payment", f"The total price is ₱{total:.2f}. Please enter payment:", parent=self.item_win)
            if payment is None:
                return 
            if payment < total:
